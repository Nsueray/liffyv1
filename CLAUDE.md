...
60. ~~Mining Import Pipeline Critical Fixes~~ ✅ DONE — (1) Infinite loop fix: catch block marks failed rows `status='failed'`, WHERE clauses exclude failed, MAX_BATCH_ITERATIONS=1000 guard. (2) Migration 045: VARCHAR(255)→TEXT for affiliations.company_name/position/city, prospects.name/company, persons.first_name/last_name. (3) sanitizeShortText/sanitizeCityField helpers reject corrupted PDF data before affiliations INSERT. Production validated: 21 rows imported in 5.5s, 0 errors. Commit 38bfc22, migration 045 applied 2026-04-28.
