...
60. ~~Mining Import Pipeline Critical Fixes~~ ✅ DONE — (1) Infinite loop fix: catch block marks failed rows `status='failed'`, WHERE clauses exclude failed, MAX_BATCH_ITERATIONS=1000 guard. (2) Migration 045: VARCHAR(255)→TEXT for affiliations.company_name/position/city, prospects.name/company, persons.first_name/last_name. (3) sanitizeShortText/sanitizeCityField helpers reject corrupted PDF data before affiliations INSERT. Production validated: 21 rows imported in 5.5s, 0 errors. Commit 38bfc22, migration 045 applied 2026-04-28. + 7c2650f UI TypeScript fix (importProgress.errors optional chaining) + e1218d9 docs (Render cache redeploy required) + retry validation 21/21 rows.
61. ~~PDF Child Process Guard~~ ✅ DONE — PDF processing (pdf-parse, pdftotext, etc.) isolated in child process via `fork()` with `--max-old-space-size=1536`. If PDF causes OOM, only child dies — worker stays alive, job marked failed. 3 paths guarded: (1) documentMiner URL PDF, (2) miningService URL PDF download, (3) miningService file upload PDF. New files: `pdfProcessGuard.js` (fork wrapper, 5min timeout, signal detection), `pdfChildWorker.js` (isolated runner). Triggered by 38MB PDF crashing 2GB Standard worker (status 139/SIGSEGV).
