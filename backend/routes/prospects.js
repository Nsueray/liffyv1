...
router.post('/api/prospects', authRequired, async (req, res) => {
  try {
    ...
    if (!prospects) return res.status(400).json({ error: "prospects array required" });
    ...
    return res.json({ success: true, inserted: result.rows.length, prospects: result.rows });
...
/**
 * GET /api/prospects
 * Query with filters: email, country, sector, verification_status, list_id
 */
router.get('/api/prospects', authRequired, async (req, res) => {
  try {
    ...
    if (list_id) {
      query += `JOIN list_members lm ON lm.prospect_id = p.id AND lm.list_id = $${idx} `;
      params.push(list_id);
      idx++;
    }

    if (email) { where.push(`p.email ILIKE $${idx}`); params.push(`%${email}%`); idx++; }
    if (country) { where.push(`p.country = $${idx}`); params.push(country); idx++; }
    if (sector) { where.push(`p.sector = $${idx}`); params.push(sector); idx++; }
    if (verification_status) { where.push(`p.verification_status = $${idx}`); params.push(verification_status); idx++; }

    query += `WHERE ${where.join(" AND ")} ORDER BY p.created_at DESC`;

    const result = await db.query(query, params);

    return res.json({ success: true, prospects: result.rows });
