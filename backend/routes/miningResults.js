...
let idx = 3;
const emailLengthExpr = `CASE  WHEN jsonb_typeof(COALESCE(mr.emails::jsonb, '[]'::jsonb)) = 'array' THEN jsonb_array_length(COALESCE(mr.emails::jsonb, '[]'::jsonb)) ELSE 0 END`;
if (has_email === 'with') {
...
if (search) {
where.push(`( COALESCE(mr.company_name, '') ILIKE $${idx} OR COALESCE(mr.contact_name, '') ILIKE $${idx} OR COALESCE(mr.website, '') ILIKE $${idx} OR COALESCE(mr.source_url, '') ILIKE $${idx} OR COALESCE((mr.emails)::jsonb::text, '') ILIKE $${idx} )`);
params.push(`%${search}%`);
idx++;
}
const whereSql = where.join(' AND ');
const resultsRes = await db.query(
`SELECT  mr.id, mr.job_id, mr.company_name, mr.contact_name, mr.job_title, mr.emails, mr.website, mr.phone, mr.country, mr.city, mr.address, mr.source_url, mr.confidence_score, mr.verification_status, mr.status, mr.created_at, mr.updated_at, mr.raw FROM mining_results mr JOIN mining_jobs mj ON mj.id = mr.job_id WHERE ${whereSql} ORDER BY mr.created_at DESC, mr.id DESC LIMIT $${idx} OFFSET $${idx + 1}`,
[...params, limit, offset]
);
const countRes = await db.query(
`SELECT COUNT(*)::int AS total FROM mining_results mr JOIN mining_jobs mj ON mj.id = mr.job_id WHERE ${whereSql}`,
params
);
...
`SELECT mr.* FROM mining_results mr JOIN mining_jobs mj ON mj.id = mr.job_id WHERE mr.id = $1 AND mj.organizer_id = $2`,
...
`UPDATE mining_results mr SET ${sets.join(', ')} FROM mining_jobs mj WHERE mr.job_id = mj.id AND mr.id = $${idx} AND mj.organizer_id = $${idx + 1} RETURNING mr.*`,
values
);
...
const organizerId = req.auth.organizer_id;
const existingRes = await db.query(
`SELECT mr.id FROM mining_results mr JOIN mining_jobs mj ON mj.id = mr.job_id WHERE mr.id = $1 AND mj.organizer_id = $2`,
[resultId, organizerId]
);
if (existingRes.rowCount === 0) {
return res.status(404).json({ error: "Result not found" });
...
await db.query(
`DELETE FROM mining_results mr USING mining_jobs mj WHERE mr.job_id = mj.id AND mr.id = $1 AND mj.organizer_id = $2`,
[resultId, organizerId]
);
